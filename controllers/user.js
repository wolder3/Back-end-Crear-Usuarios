...
function signUp(req, res) {
  ...
  if (!password || !repeatPassword) {
    res.status(404).send({ message: "Las constrasenas son obligatorias." });
  } else {
    if (password !== repeatPassword) {
      res
        .status(404)
        .send({ message: "Las constrasenas tienen que ser iguales." });
    } else {
      ...
      if (!userData) {
        ...
        if (req.files) {
          ...
          if (fileExt !== "png" && fileExt !== "jpg") {
            res.status(400).send({
              message:
                "La extension de la imagen no es valida. (Extensiones permititidas: .png y .jpg)",
            });
          } else {
...
function signUpAdmin(req, res) {
  ...
  if (!password) {
    res.status(500).send({ message: "La contrasena es obligatoria" });
  } else {
        ...
        user.save((err, userStored) => {
          if (err) {
            ...
            if (!userStored) {
              res
                .status(500)
                .send({ message: "Error al crear al nuevo usuario." });
            } else {
              // res.status(200).send({ user: userStored });
              res
                .status(200)
